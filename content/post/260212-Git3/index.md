...
### Stash
이제부터는 필수는 아니지만 알면 좋은 기능들에 대해서 설명해보겟습니다.  
Stash는 임시로 저장하는 물양동이 같은 개념으로 Local에서 임시로 작업물을 따로 저장하는 기능입니다.  

다른 브랜치로 옮길려면 작업내역에 아무것도 없어야만 성공적으로 브랜치를 옮길 수 있습니다.  
그렇기에 이 Stash를 통해 작업내역들을 잠시 저장했두고 브랜치를 옮길 일이 일어날 수 있습니다.  
![]({23BF10B2-7AEC-4185-86B6-463F84945767}.png)
저는 지금 Master 브랜치에서 파일을 수정을 해야했습니다.  
하지만, gh-page라는 브랜치에서 해당작업을 진행했네요.  
이 작업을 Stash에 담아보겠습니다.  

![]({98849CBD-2A99-4CA0-9C4B-E59F73A74E12}.png)
![]({A3676239-3D31-46FB-ABE3-33DD2E95C0D7}.png)

Local Changes 영역에서 필요한 파일들을 Ctrl+좌클릭으로 하나씩 지정 후,  
우클릭으로 Stash를 행동해주시고, 그림과같이 원하는대로 이름 지어주세요.  
![]({3FB03B2E-DF9D-4E65-B187-EB4073638E1A}.png)  
저장된 작업내역을 가져올 브랜치에서 왼쪽탭에 있는 Stash를 Apply를 진행해주세요.

### Git Reset - 커밋 취소
![]({08F6E524-F65A-4903-80E0-BD860F763E04}.png)
Reset은 쓸 일이 보통 없는데, 저는 Commit을 취소할 떄 종종 사용합니다.  
그 외에는 거의 쓰지 않는 편입니다.
```bash
